...
### Hook 4: `journal-check.sh`
- **Event**: PostToolUse (async, matcher: `Bash`, if: `Bash(git commit *)`)
- **Purpose**: Remind about journal freshness after commits
- **Behavior**: Read first 20 lines of `docs/development-journal.md`, check if today's date appears. If stale, output reminder.
- **Advisory only**: Never blocks. Just reminds.
    ...
    "PostToolUse": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .claude/hooks/journal-check.sh",
            "async": true
          }
        ]
      }
...
4. Test: Invoke each skill, verify they chain correctly
5. **Commit**: `LIFE-1.4: Add start-project, start-work, and done lifecycle skills`
...
6. **Commit**: `LIFE-1.7: Documentation and end-to-end validation`
